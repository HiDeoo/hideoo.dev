...
This guide will walk you through the process of adding support for user configuration in a Starlight plugin, allowing users to customize the plugin's behavior through their Starlight configuration.
...
Using a function allows you to accept user-supplied configuration options as arguments, which can then be used to customize the plugin's behavior.
...
export default function starlightPluginExample(
  ...
  return {
    ...
    hooks: {
      'config:setup'() {
        // Do the default tasks of the plugin.
        // ...

        // Read the user-supplied plugin configuration, if provided.
        if (userConfig?.useConfetti) {
          // Enable confetti effect if the user has enabled it.
          // ...
        }
      },
    },
...
:::note
In the example above, the `userConfig` argument is optional, allowing users to use the plugin without providing any configuration options.
...
Using Zod to validate user configuration options not only provides data validation and type safety but also allows you to define custom error messages and hints, making it easier for users to understand what went wrong and how to fix the issue.
