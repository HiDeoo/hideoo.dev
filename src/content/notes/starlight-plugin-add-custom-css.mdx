...
publishDate: 2025-06-01 22:00:00
notebook:
...
This guide will walk you through the process of adding custom CSS in a Starlight plugin, allowing you to customize the styles applied to a Starlight website.

:::note[Heads up]
Are you looking to create a theme for Starlight instead?
Check out the [“How to create a Starlight theme”](/notes/starlight-plugin-how-to-create-theme) note instead to get started immediately.
:::
...
Starlight provides a [`customCss`](https://starlight.astro.build/reference/configuration/#customcss) configuration option to add additional CSS files to a Starlight website.
...
The following example adds a new entry point name `./styles.css` that points to a `styles.css` file in the plugin's root directory:
...
To reference the previously exported CSS file in the Starlight configuration, the `starlight-plugin-example/styles.css` identifier can be used in the `customCss` option of the Starlight configuration.
...
The following example updates the [`customCss`](https://starlight.astro.build/reference/configuration/#customcss) option in the user-supplied Starlight configuration to prepend the plugin's custom CSS file:
  ...
  return {
    ...
    hooks: {
      'config:setup'({ config, updateConfig }) {
        ...
        updateConfig({
          customCss: [
            'starlight-plugin-example/styles.css',
            ...currentCustomCss,
          ],
        })
      },
...
After creating the `styles.css` file in the plugin's root directory, you can add your custom CSS styles to it.
